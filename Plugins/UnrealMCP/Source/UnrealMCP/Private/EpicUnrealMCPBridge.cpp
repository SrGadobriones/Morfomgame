    ...
    AsyncTask(ENamedThreads::GameThread, [this, CommandType, Params, Promise = MoveTemp(Promise)]() mutable
    {
        ...
        try
        {
            ...
            else if (CommandType == TEXT("execute_python"))
            {
                ...
                // Capture log output while Python runs. UE's `py` console command
                // forwards print() to LogPython so we grab those lines.
                ...
                public:
                    ...
                    {
                        FString C = Category.ToString();
                        if (C.Equals(TEXT("LogPython"), ESearchCase::IgnoreCase))
                        {
                            Captured += FString(V) + TEXT("\n");
                        }
                    }
                } LogCapture;
                GLog->AddOutputDevice(&LogCapture);

                // Execute via IPythonScriptPlugin (supports multi-line code)
                ...
                if (PyPlugin && PyPlugin->IsPythonAvailable())
                {
                    ...
                    bool bOk = PyPlugin->ExecPythonCommandEx(PyCmd);
                    ResultJson->SetBoolField(TEXT("success"), bOk);
                    if (!PyCmd.CommandResult.IsEmpty())
                    {
                        ResultJson->SetStringField(TEXT("result"), PyCmd.CommandResult);
                    }
                }
                else
                {
                    ResultJson->SetBoolField(TEXT("success"), false);
                    ResultJson->SetStringField(TEXT("error"), TEXT("Python plugin not available"));
                }

                GLog->RemoveOutputDevice(&LogCapture);

                ResultJson->SetBoolField(TEXT("success"), true);
                ResultJson->SetStringField(TEXT("output"), LogCapture.Captured);
            }
