...
UK2Node* FBlueprintNodeManager::CreateCallFunctionNode(UEdGraph* Graph, const TSharedPtr<FJsonObject>& Params)
{
	...
	// Support external class member functions via optional "target_class" param.
	// Accepts short class name ("KismetMathLibrary", "SpringArmComponent"),
	// or full path ("/Script/Engine.KismetMathLibrary").
	FString TargetClassStr;
	bool bSetExternal = false;
	if (Params->TryGetStringField(TEXT("target_class"), TargetClassStr) && !TargetClassStr.IsEmpty())
	{
		...
		if (!TargetClass)
		{
			TargetClass = UClass::TryFindTypeSlow<UClass>(TargetClassStr);
		}
		if (!TargetClass)
		{
			...
			{
				TargetClass = LoadObject<UClass>(nullptr, *(FString(P) + TargetClassStr));
